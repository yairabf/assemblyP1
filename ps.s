...
replaceChar:
  ...
  movq    %rdi, %r12              #address of string into r12
  movb    (%r12), %r15b           #for the counter i get the number of char in the string
  addq    $1, %r12                #move from the length of the string to the first char of string
  movb    %sil, %r13b             #old char into sil
  movb    %dl, %r14b              #new char into dl
  .checkNext:                     #if they not match go to check the next char
  subq    $1, %r15                #sub from the counter 1
  cmpb    %r13b, (%r12)           #check if the chars equale
  je      .goReplaceChar          #if yes replace the char
  cmpq    $0, %r15                #if not check if we need to end the loop
  jle      .returnFromFunction    #if we at the end of loop->we end to endle the string go out
  incq    %r12                    #if current char of string is not eqale to oldchar countinue to next char string(if we have more)
  jmp     .checkNext              #jump to start of the loop

  .goReplaceChar:                 #if we need to replace them we get to here
  movb    %r14b, (%r12)           #swap newchar with char of string
  incq    %r12                    #for sending the next char of string to check
  jmp     .checkNext              #after replace go back to loop for next char

  .returnFromFunction:
  popq   %r15                     #we need to backup r12 for saving him
  popq   %r14                     #we need to backup r13 for saving him
  popq   %r13                     #we need to backup r14 for saving him
  popq   %r12                     #we need to backup r15 for saving him
  ret
